...
def show_help():
    ...
    print("  python main.py --days NUMBER [--output FILENAME] [--reauth]\n")
    ...
    print("  --reauth          Force re-authentication (use a different account)")
    print(f"{Fore.BLUE}Examples:{Style.RESET_ALL}")
    print("  python main.py --days 45")
    print("  python main.py --days 90 --output quarterly_report.xlsx")
    print("  python main.py --reauth --days 7\n")
    ...
    print(f"{Fore.BLUE}Note on Accounts:{Style.RESET_ALL}")
    print("  - The script pulls data from the account used during authorization.")
    print("  - Use --reauth to switch to a different account.\n")
...
def main():
    ...
    # Handle default filename with date and days
    if args.output is None and args.days:
        today_str = datetime.now().strftime('%Y-%m-%d')
        args.output = f"{today_str}-{args.days}d-calendar_export.xlsx"
    
    # Handle re-authentication
    if args.reauth and os.path.exists('token.pickle'):
        try:
            os.remove('token.pickle')
            print(f"{Fore.BLUE}Existing token deleted. Forcing re-authentication...{Style.RESET_ALL}")
        except Exception as e:
            print(f"{Fore.RED}Error deleting token.pickle: {e}{Style.RESET_ALL}")
